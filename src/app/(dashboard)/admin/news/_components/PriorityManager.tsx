...
import { NewsPriority, NewsService } from "@/services/news.service";
...
export default function PriorityManager() {
    ...
    const [currentPriority, setCurrentPriority] = useState<Partial<NewsPriority>>({ name: "", level: "" });
    ...
    const fetchPriorities = async () => {
        setIsLoading(true);
        try {
            const data = await NewsService.getPriorities();
    ...
    const handleCreate = () => {
        setCurrentPriority({ name: "", level: "" });
        setIsEditing(false);
        setIsModalOpen(true);
    };

    const handleEdit = (priority: NewsPriority) => {
    ...
    const handleDelete = async (id: number) => {
        if (confirm("Are you sure you want to delete this priority?")) {
            try {
                await NewsService.deletePriority(id);
                fetchPriorities();
            } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            if (isEditing && currentPriority.id) {
                await NewsService.updatePriority(currentPriority.id, currentPriority);
            } else {
                await NewsService.createPriority(currentPriority);
            }
            ...
            <div className="max-w-full overflow-x-auto">
                <table className="w-full table-auto">
                    ...
                    <tbody>
                        {isLoading ? (
                            <tr>
                                <td colSpan={3} className="px-4 py-5 text-center"><Loader /></td>
                            </tr>
                        ) : (
                                ...
                                <form onSubmit={handleSubmit}>
                                    <div className="mb-4">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setCurrentPriority({ ...currentPriority, name: e.target.value })}
                                        />
                                    </div>
                                    <div className="mb-4">
                                        <label className="mb-2.5 block font-medium text-black dark:text-white">
                                        ...
                                        <input
                                            type="text"
                                            placeholder="e.g. Normal, Urgent"
                                            className="w-full rounded-lg border border-stroke bg-transparent py-2 pl-5 pr-5 outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                                            value={currentPriority.level}
                                            onChange={(e) => setCurrentPriority({ ...currentPriority, level: e.target.value })}
                                        />
                                    </div>
